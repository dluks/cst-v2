#!/usr/bin/env python
"""
Entry point script to generate Slurm jobs for building moment maps.
...
def cli() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Submit Slurm jobs to build statistical moment maps (mean, variance, "
            "skewness, kurtosis) for all configured traits."
        )
    )
    parser.add_argument(
        "-s",
...
def run_trait_job(
    trait: str, source: str, params_path: str | None, overwrite: bool
) -> tuple[str, int]:
    """Run a single trait job locally."""
    cmd = build_base_command(
        "src.data.build_moment_map",
        params_path=params_path,
        overwrite=overwrite,
        extra_args={"--trait": trait, "--source": source},
    )
...
def run_local(
    ...
    print(
        f"\nRunning {len(trait_names)} traits ({source}) locally with {n_jobs} parallel jobs"
    )

    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        # Submit all jobs
        futures = {
            executor.submit(run_trait_job, trait, source, params_path, overwrite): trait
...
def run_slurm(
    ...
    for trait in trait_names:
        ...
        slurm = Slurm(
            job_name=f"moment_{source}_{trait}",
...
def main() -> None:
    ...
    if use_local:
        ...
        run_local(
            trait_names, args.source, str(params_path), args.overwrite, args.n_jobs
        )
    else:
        ...
        # Submit to Slurm
        log_dir = setup_log_directory(f"build_moment_maps_{args.source}")
